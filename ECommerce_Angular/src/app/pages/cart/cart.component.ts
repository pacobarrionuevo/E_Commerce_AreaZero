  ...
  export class CartComponent implements OnInit {
    ...
    pagar() {
      // Recupera el ID de la orden temporal desde localStorage o tu servicio
      const orderId = localStorage.getItem('ordenId'); // Cambia si lo guardas de otra forma
    
      if (!orderId) {
        console.error('No se encontró el ID de la orden temporal.');
        return;
      }
    
      // Redirige a la página de checkout con el método de pago
      this.router.navigate(['checkout'], {
        queryParams: { orderId, metodoPago: 'stripe' } // Método de pago puede cambiarse
      });
    }
